...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', 
                                             options=[{'label': site, 'value': site} for site in set(spacex_df['Launch Site'].values)] + [{'label': 'All Sites', 'value': 'ALL'}],
                                             value='ALL',
...
def get_pie_chart(entered_site: str):
    if entered_site == 'ALL':
        ...
        successes = filtered.groupby(by=['class']).size().reset_index(name='Count')
        fig = px.pie(successes,
                     values='Count',
                     names='class',
